			...
			public void keyPressed(KeyEvent e)
			{
                ...
                addMouseListener(new MouseListener() {
                    ...
                    public void mouseClicked(MouseEvent me) {
                        ...
                        if( x >= 545 && x <= 670 && y >=10 & y <= 30 ){
                            poder();
                            System.out.println("Utilizando poder...");
                        }
                        if( x >= 480 && x <= 715 && y >=280 & y <= 300 ){
                            eliminarColumnaAleatoria();
                            System.out.println("Eliminando una columna aleatoria de puyos...");
                        }
                        if( x >= 510 && x <= 700 && y >=330 & y <= 350 ){
                            cargarEstado();
                            System.out.println("Cargando último estado guardado...");
                        }
                        if( x >= 500 && x <= 700 && y >=385 & y <= 400 ){
                            guardarEstado();
                            System.out.println("Guardando estado actual...");
                        }
                        if( x >= 490 && x <= 705 && y >=430 & y <= 450 ){
                            consultarPuntajesAltos();
                            System.out.println("Mostrando puntajes altos...");
                        }
                        if( x >= 510 && x <= 690 && y >=530 & y <= 550 ){
                            acelerar();
                            System.out.println("Aumentando velocidad de los puyos...");
                        }
                        if( x >= 505 && x <= 680 && y >=485 & y <= 500 ){
                            frenar();
                            System.out.println("Disminuyendo la velocidad de los puyos...");
                        }
                        if( x >= 10 && x <= 300 && y >=10 & y <= 40 ){
                            guardarPuntajesAltos();
	...
	public void paint(Graphics g)
	{
		...
		if(gameOver)//if game is over dim the game by using alpha composite and display the information
		{
			...
			g2.drawString("Game Over",len*3/2,rows*len/2);
                        g2.drawString("Guardar Puntaje", 10,40);
			g2.setPaint(Color.blue);
			g2.setFont(new Font("Ariel",Font.PLAIN,len/3));
		...
		if(paused)//if game is paused dim the game by using alpha composite and display the information
		{
			...
			g2.drawString(" Press <Enter> to restart the Game",len/2,(rows+3)*len/2);
                ...
                g2.setFont(new Font("Ariel", Font.PLAIN,len/4));
                g2.drawString("Aumentar Velocidad", 510, 550);
                g2.drawString("Disminuir Velocidad", 510, 500);
                g2.drawString("Consultar Puntajes Altos", 490, 450);
                g2.drawString("Utilizar Poder", 550, 30);
                g2.drawString("Guardar Estado Actual", 500, 400);
                g2.drawString("Cargar último estado", 510, 350);
                g2.drawString("Eliminar Columna Aleatoria", 480, 300);
	}	
        
        public void acelerar(){
            int delay = 200;
            if(timer.getDelay() > 200){
                timer.setDelay(timer.getDelay()-delay);
            }
        }
        ...
        public void guardarEstado(){
            ...
            try {
            RandomAccessFile archivo = new RandomAccessFile("score.txt", "rw");
            RandomAccessFile archivoMatriz = new RandomAccessFile("game.txt", "rw");
            archivo.setLength(0);
            archivoMatriz.setLength(0);
            int started = this.started? 0 : 1;
            int paused = this.paused? 0 : 1;
            int levelFlag = this.levelflag? 0 : 1;
            int reached = this.reached? 0 : 1;
            archivo.writeBytes(this.level + "," + this.pieces + "," + this.score + "," + started + "," + paused + "," + levelFlag + "," + reached);
            for(int i = 0 ; i < scr.length ; i++){
                for(int j = 0 ; j < scr[i].length ; j++){
                    if(j+1 != scr[i].length){
                        archivoMatriz.writeBytes(scr[i][j]+",");
                    }else{
                        archivoMatriz.writeBytes(scr[i][j]+ "\r\n");
                    }
                }
            ...
            Logger.getLogger(Puyo_Puyo.class.getName()).log(Level.SEVERE, null, ex);
        ...
        public void cargarEstado(){
            ...
            RandomAccessFile archivoMatriz;
            try {
                archivo = new RandomAccessFile("score.txt", "r");
                archivoMatriz = new RandomAccessFile("game.txt", "r");
                String lineaActual = "";
                while((lineaActual = archivo.readLine()) != null){
                String[] nums = lineaActual.split(",");
                this.level = Integer.parseInt(nums[0]);
                this.pieces = Integer.parseInt(nums[1]);
                this.score = Integer.parseInt(nums[2]);
                this.started = nums[3].equals("0") ? true : false;
                this.paused = nums[4].equals("0") ? true : false;
                this.levelflag = nums[5].equals("0") ? true : false;
                this.reached = nums[6].equals("0") ? true : false;
            }
                lineaActual="";
                int contadorAuxiliar = 0;
                while((lineaActual = archivoMatriz.readLine()) != null){
                    String[] nums = lineaActual.split(",");
                    for(int j = 0 ; j < scr[0].length ; j++){
                        scr[contadorAuxiliar][j] = Integer.parseInt(nums[j]);
                    }
                    contadorAuxiliar++;
                }
            } catch (FileNotFoundException ex) {
        ...
        public void guardarPuntajesAltos(){
            String nombre = JOptionPane.showInputDialog("Ingrese el nombre del jugador");
            db.validarIngreso(nombre, this.score);
        }
        
        public void consultarPuntajesAltos(){
            db.consultarPuntajes();
        }
        
        public void poder(){
            int puntaje = this.score;
            init();
            generatePuyos();
            clips[0].loop();
            started=false;
        ...
        public void eliminarColumnaAleatoria(){
            Random r = new Random();
            int columna = r.nextInt(cols-1) + 1;
            for( int i = 0 ; i < scr.length ; i++ ){
                scr[i][columna] = 0;
            }
            repaint();
